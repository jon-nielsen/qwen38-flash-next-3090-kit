...
if TYPE_CHECKING:
    ...
    VLLM_PLE_FP8_EMBEDDING: bool = False
    VLLM_QSA_KV_SCALES: str | None = None
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Opt-in FP8 PLE n-gram table for mixed compressed-tensors checkpoints
    # that ignore PLE in quant_config but ship float8_e4m3fn shards.
    # Accepts 1/true/yes (case-insensitive); default off.
    "VLLM_PLE_FP8_EMBEDDING": lambda: (
        os.getenv("VLLM_PLE_FP8_EMBEDDING", "0").strip().lower() in ("1", "true", "yes")
    ),
    # Path to a JSON sidecar of per-layer static QSA main-KV E4M3 scales.
    # Unset: exact no-op (layers keep the vLLM default scale of 1.0).
    "VLLM_QSA_KV_SCALES": lambda: os.getenv("VLLM_QSA_KV_SCALES"),
...
def compile_factors() -> dict[str, object]:
    ...
    ignored_factors: set[str] = {
        ...
        # Path-only sidecar of QSA K/V scales; loaded after weights, not a
        # compile-graph input.
        "VLLM_QSA_KV_SCALES",
        "VLLM_HOST_IP",
