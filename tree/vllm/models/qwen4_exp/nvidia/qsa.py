...
class Qwen4ExpQSAFlashAttentionImpl(FlashAttentionImpl):
    ...
    def __init__(self, *args, **kwargs) -> None:
        # QSA never dispatches to a FlashAttention kernel: attention runs through
    ...
    def forward_qsa(
        ...
        # Announce the effective scales once per layer. load_qsa_static_kv_scales()
        # is an explicit integration call, so a deployment that forgets it runs at
        # the vLLM default of 1.0 -- which is a legitimate configuration but must
        # never be a silent one. This makes the actual values greppable per rank.
        ...
        *,
        ...
        if kv_cache_scheme is not None:
            raise NotImplementedError(
                ...
                "Dynamic, per-head, per-token, and other non-scalar KV scales "
                "are unsupported; use static scalar per-layer K/V scales from "
        ...
        # The checkpoint has no KV scale scheme. These registered scalar
        # buffers therefore remain 1.0 unless explicitly replaced after
        # weight loading by load_qsa_static_kv_scales().
...
def load_qsa_static_kv_scales(
    ...
    *,
    ...
    # PP-aware strictness: with pipeline parallelism each rank holds only its
    # stage's QSA layers, so sidecar entries for OTHER stages' layers are
    # expected here and are applied on the ranks that own them. Names that
    # match this model's QSA-layer family (layers.N.self_attn.attn) but are
    # not on this rank are pipeline-stage siblings: ignore them, loudly. The
    ...
    if stage_siblings:
        logger.info(
            "Qwen4Exp QSA: ignoring %d sidecar entries for QSA layers not on"
            " this rank (pipeline-parallel stage split): %s%s",
            len(stage_siblings),
            ", ".join(stage_siblings[:4]),
            " ..." if len(stage_siblings) > 4 else "",
        )
    ...
    for layer_name, entry in raw.items():
        if layer_name not in qsa_layers:
            # pipeline-stage sibling (logged above); applied on its own rank
            continue
